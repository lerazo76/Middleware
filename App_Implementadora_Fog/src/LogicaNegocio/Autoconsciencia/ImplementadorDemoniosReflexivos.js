...
const ImplmentadorFunciones = require("../Autoconsciencia/ImplementadorFunciones"); //REVISAR LO DE LAS FUNCIONES SI SE IMPLEMENTAN YA COMO CODIGO O NO
...
let reflexivosMap = [];
const server = require("../../Comunicacion/Autoconciencia/ImplementadorServicios");
const request = require("request");

async function ejecutarDemoniosControlados() {
  await mapearUnNivel();
  await ImplementarFuncionDemonio(reflexivosMap);
  await ImplementarDemonio(reflexivosMap);
...
async function ImplementarFuncionDemonio(procreflex){
  procreflex.forEach(element =>{
    if(element.hasOwnProperty("funcionName")){
      console.log('\x1b[34m%s\x1b[0m',"Función RE " + element["funcionName"]+ " implementada (OK)");
    }
    else if(element.hasOwnProperty("funcionServicioName")){
      console.log('\x1b[34m%s\x1b[0m',"Servicio web RE" + element["funcionServicioName"]+ " implementado (OK)")
    }
  });
}
      ...
      setInterval(async () => {
        try {
          ...
          if(element['aidType'] === "individual"){
                    ...
                    async (error, response, body) =>{
                      if(!error && response.statusCode ===200){
                        ...
                        if(guardarMetricaIndirecta && guardarIndicador){
                          ...
                          console.log('\x1b[34m%s\x1b[0m',element["nombre"] + " \tTipo: Reflexivo - Estado " +"\tFecha: " + fechaformato);
            ...
            let arraymetricas = []; //arreglo de metricas que se usara luego como parametro para el modelo LSP
            ...
            if(element["PesosTablas"] && element["operadorAgregacion"]){
              ...
              async function obtenerPesoMetrica(item){
                ...
                const metrica = await GestorBD.obtenerMetricaBDM(bd,tabla,column); //obtener la ultima metrica guardada de los procesos hijos
                arraypesos.push(peso);
                arraymetricas.push(metrica);
              }
              (async () => {
  ...
  procesos.forEach((element) => {
    ...
    if (Object.keys(element).length > 1 && element["captures"]["$"]["xsi:type"] === "individual") {
      ...
      if (element["usesCalculationMethod"][0]["implementationResourceType"].toLowerCase() === "servicio") {
        ...
        console.log("Conex bd servicio ", conexbd);
        ...
        server.startServer(collector["endPoint"], collector["funcionServicio"], collector["puertoServicio"]); //inicia el servidor REVISAR
      }
    }else{ // caso en el que el proceso (aspecto) sea colectivo (se puede mejorar el codigo para no repetir codigo) 
      collector = {
        bd: "postgresqllocal", //la base de datos si puede cambiar ya que puede estar en otro nodo (postresqlcloud o postresqllocal)
        tabla: "metricas", //para los tipo funcion siempre va a tomar de la tabla metricas
        propiedad : '', 
      ...
      if(element["captures"]['isDerivedFrom']){
        ...
        //mapeo de bds
        let columpath = item["isCaptured"]["usesCollectionMethod"][0]["columns_paths"];
        let BDrelacionada = GestorBD.mapearconexionBD(columpath, modelo);
        let conexbd = BDrelacionada[0]["name"];
        conexbd = conexbd.trim();
        let conextabla = BDrelacionada[1]["name"];
        let conexcolum = BDrelacionada[2]["name"];
        objetorutasbds.peso = peso;
        objetorutasbds.bd = conexbd.toLowerCase();
        objetorutasbds.tabla = conextabla.toLowerCase();
        objetorutasbds.column = conexcolum.toLowerCase();
        arrayrutasbds.push(objetorutasbds);
        }));
      }
      collector["PesosTablas"] = arrayrutasbds;
    }
