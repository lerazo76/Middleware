            ...
            if (err) {
                ...
                client.connect(err => {
                    if (err) {
                        ...
                        console.log('\x1b[32m%s\x1b[0m', 'Implementando la estructura de las bases de datos que almacenan los datos de monitoreo');
                        console.log('\x1b[32m%s\x1b[0m', `Base de datos ${x[x.length - 1]} implementada (ok)`);
                                    ...
                                    dataStore.containsDataTable.forEach(async tabla => {
                                        await client.query(crearTbPostgres(tabla), err, res => {
                                            if (err) {
                                                ...
                                                console.log('\x1b[32m%s\x1b[0m', `Tabla ${nombreTabla} implementada (ok)`);
                                            }
                                        })
                                    })
                                }
                ...
                let x = uri.split("/");
                dataStore.containsDataTable.forEach(async tabla => {
                    await bd.client.query(crearTbPostgres(tabla), err, res => {
                        if (err) {
                            throw err;
                        } else {
                            let nombreTabla = tabla.name
...
//funcion para obtener el nodo de computacion (caso autoconsciencia solo se crea la tabla metricas en el nodo fog)
function isFogNode(jsonData) {
    for (const item of jsonData) {
        if (item["xsi:type"] === "MonitorIoT:FogNode") {
...
async function crearTablaMetricas(basedatos) {
    ...
    const isfogNode = isFogNode(modelo);
    if (isfogNode) {
        try {
            ...
            if(tipoApp==2 || tipoApp==0){
                console.log("\x1b[34m%s\x1b[0m","Implementando/reconfigurando la estructura de las bases de datos que almacenan las metricas de autoconsciencia...");
                console.log("\x1b[34m%s\x1b[0m","Tabla MetricaAutoconsciencia implementada (OK)");
            }
