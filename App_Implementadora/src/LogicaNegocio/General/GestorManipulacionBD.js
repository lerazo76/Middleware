...
// const funciones = require("../../../../App_Supervisora/Funciones");
...
async function obtenerIndicadorBDA(bd, procesoid){
  const client = new Client({
    user: "postgres",
    // host: "localhost",
    host: "db-nodo-fog-1",
  ...
  try {
    ...
    SELECT valor
    FROM metricas
    WHERE pid = $1 AND tipo = 'indicador'
    ORDER BY fecha DESC
    LIMIT 1;
  `;
    // const consulta = 'SELECT valor '+' FROM metricas'+' WHERE pid = ' +procesoid+ ' AND tipo = \'indicador\'  ORDER BY fecha DESC LIMIT 1;';
    // const resultado = await client.query(consulta,[procesoid]);
    const resultado = await client.query(
      'SELECT valor FROM metricas WHERE pid = $1 AND tipo = $2 ORDER BY fecha DESC LIMIT 1',
      [procesoid, 'indicador']
    );
    // Verificar si se obtuvieron resultados
    if (resultado.rows.length > 0) {
