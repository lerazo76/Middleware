...
if (tipoApp==1 || tipoApp==0){
    console.log('\x1b[32m%s\x1b[0m', 'Implementando las funciones APIs ...');
}
...
modelo.forEach((resource, index) => {
    ...
    resource.containsResource.forEach((propiedades)=>{
        if(propiedades['xsi:type'] == 'MonitorIoT:API'){
            ...
            eval(nuevaAPI);
            if (tipoApp==1 || tipoApp==0){
                console.log('\x1b[32m%s\x1b[0m', `API ${propiedades.name} implementada (OK)`);
            }
        }
    })
});
...
exports.getTiempo = function(globalConfig,api){
    ...
    if(api.$.hasLinkAPIToIoTDevice){
        ...
        //containsLink.9
        linkdataf=globalConfig["ArchitectureSelfAwarenessIoT"].containsLink[9].$.supports;
    }
