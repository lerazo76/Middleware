const EstructuradorProcesos = require("../Autoconsciencia/EstructuradorProcesos"); //importar el archivo que obtiene y clasifica los procesos de autoconsciencia
const ImplmentadorFunciones = require("../Autoconsciencia/ImplementadorFunciones"); //importar el archivo con las funciones (eval) que utlizan los procesos de autoconsciencia  
const GestorBD = require("../General/GestorManipulacionBD"); //importar el archivo con las funciones para el manejo de las BD
let procesos = EstructuradorProcesos.prereflexivos; //recuperar solo los procesos pre-reflexivos
let entidad = EstructuradorProcesos.entidad; //recuperar la entidad sobre la que se esta trabajando
let modelo = EstructuradorProcesos.modelo; //recuperar el modelo de arquitectura 
const start = require('../../../StartApp');
const tipoApp = start.app;


let preReflexivosMap = []; //array para almacenar los procesos pre-reflexivos 'mapeados', solo con la informacion necesaria 
//funcion para controlar la ejecucion de los demonios
async function ejecutarDemoniosControlados(nodoComputacion) { 
  await mapearUnNivel();
  await ImplementarFuncionDemonio(preReflexivosMap, nodoComputacion);
  if (tipoApp==2 || tipoApp==0 || tipoApp==0){
    console.log("");
    console.log('\x1b[32m%s\x1b[0m',"Implementando/reconfigurando los demonios de los procesos de autonsciencia...");
  }
  await ImplementarDemonio(preReflexivosMap, nodoComputacion);
...
async function ImplementarFuncionDemonio(procprereflex, nodoComputacion){
  if (tipoApp==2 || tipoApp==0){
  console.log('\n\x1b[32m%s\x1b[0m',"Implementando/reconfigurando los recursos de implementación de los procesos de autconsciencia...")
  }
    ...
    setInterval(async () => {
      try {
        ...
        if(metricaDirecta && guardarMetricaDirecta && guardarIndicador){
          ...
          if (tipoApp==2 || tipoApp==0){
          console.log('\x1b[34m%s\x1b[0m',element["nombre"] + " \tTipo: Pre-reflexivo - " + element["aidType"] +" \tFecha: " + fechaformato);
  ...
  procesos.forEach((element) => {
    ...
    if (Object.keys(element).length > 1) {
      ...
      collector = {
        ...
        aidType: element["captures"]["$"]["type"], //si es ESTADO O PREDCCION en el caso del modelo utilizado
